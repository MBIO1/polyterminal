// Returns a bash heredoc script that writes bot.mjs directly on the droplet.
// No auth needed — the bot source is not sensitive.
...
Deno.serve(async (req) => {
  try {
    ...
    const botSource = `// Droplet WebSocket arbitrage bot — BASIS-CARRY v3 (BIDIRECTIONAL + CROSS-VENUE CONFIRMATION)
...
const PAIRS = (process.env.PAIRS || 'DOGE-USDT,ADA-USDT,APT-USDT,XRP-USDT,SOL-USDT').split(',');
...
const MIN_FILLABLE_USD  = Number(process.env.MIN_FILLABLE_USD || 1);
...
if (!INGEST_URL || !TOKEN) {
  console.error('Missing BASE44_INGEST_URL or TOKEN in .env');
  process.exit(1);
}
...
function connectOKX() {
  ...
  ws.on('open', () => {
    ...
    for (const p of PAIRS) {
      args.push({ channel: 'tickers', instId: p });
      args.push({ channel: 'tickers', instId: p + '-SWAP' });
    }
  ...
  if (spotVenues.length >= 2) {
    ...
    if (cheapest.name !== richest.name) {
      ...
      if (netBps >= threshold) {
        ...
        const last = recentlyPosted.get(key);
        if (fillable >= MIN_FILLABLE_USD && signalAgeMs <= MAX_SIGNAL_AGE_MS && !(last && now - last < 5000)) {
          recentlyPosted.set(key, now);
          stats.posted++;
          post({
            pair, asset: pair.split('-')[0],
        ...
        } else if (last && now - last < 5000) stats.rejected_dedupe++;
...
console.log(
  ...
  '  binance: ' + (DISABLE_BINANCE ? 'DISABLED' : 'enabled') + '\\n' +
...
if (!DISABLE_BINANCE) {
  ...
  console.log('  [Binance] skipped — geo-blocked. Set DISABLE_BINANCE=false to re-enable.');
}
refreshThresholds();
`;

    // Return as a bash script that writes the file via heredoc
    const script = `#!/bin/bash
set -e
cat > /opt/arb-bot/bot.mjs << 'BOTEOF'
${botSource}
BOTEOF

echo "✅ bot.mjs written ($(wc -l < /opt/arb-bot/bot.mjs) lines)"
pm2 stop arb-bot 2>/dev/null; pm2 delete arb-bot 2>/dev/null
cd /opt/arb-bot && pm2 start bot.mjs --name arb-bot
sleep 3
pm2 logs arb-bot --lines 10 --nostream
`;

    return Response.json({ script, line_count: botSource.split('\n').length });
  } catch (error) {
