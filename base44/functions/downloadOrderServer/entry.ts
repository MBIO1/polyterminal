...
// Inline the canonical order-server v4 source so curl deployments always get the latest.
const ORDER_SERVER_SOURCE = `import 'dotenv/config';
...
const ERR = {
  ASSET_NOT_ALLOWED:'ASSET_NOT_ALLOWED', MISSING_PRICE:'MISSING_PRICE', MISSING_QTY:'MISSING_QTY',
  MIN_QTY_VIOLATION:'MIN_QTY_VIOLATION', MIN_NOTIONAL_VIOLATION:'MIN_NOTIONAL_VIOLATION',
  INSTRUMENT_FETCH_FAILED:'INSTRUMENT_FETCH_FAILED', ORDER_REJECTED:'ORDER_REJECTED',
...
const STATIC_SPECS = {
  BTCUSDT:{spot:{tickSize:0.01,pricePrecision:2,qtyStep:'0.000001',minQty:0.000048,minNotionalUsd:1},linear:{tickSize:0.10,pricePrecision:1,qtyStep:'0.001',minQty:0.001,minNotionalUsd:1}},
  ETHUSDT:{spot:{tickSize:0.01,pricePrecision:2,qtyStep:'0.0001',minQty:0.000458,minNotionalUsd:1},linear:{tickSize:0.01,pricePrecision:2,qtyStep:'0.01',minQty:0.01,minNotionalUsd:1}},
};
...
async function getInstrumentSpec(category,symbol) {
  if (!ALLOWED_SYMBOLS.has(symbol)) throw new ExecError(ERR.ASSET_NOT_ALLOWED,symbol+' not allowed');
  ...
  try {
    ...
    const inst=json.result?.list?.[0]; if (!inst) throw new Error('not_found');
    ...
    instrumentCache.set(key,spec); return spec;
  } catch(e) { const fb=(STATIC_SPECS[symbol]||{})[category]; if (fb){const spec={ts:Date.now(),...fb};instrumentCache.set(key,spec);return spec;} throw new ExecError(ERR.INSTRUMENT_FETCH_FAILED,key+': '+e.message); }
}
...
async function normalizeAndValidateOrder(category,symbol,rawQty,rawPrice) {
  ...
  if (normQty<spec.minQty) throw new ExecError(ERR.MIN_QTY_VIOLATION,'qty '+normQty+' < minQty '+spec.minQty);
...
async function bybitOrder({category,symbol,side,qty,timeoutMs=8000}) {
  ...
  const res=await fetch(BYBIT_BASE+'/v5/order/create',{method:'POST',headers:{'X-BAPI-API-KEY':API_KEY,'X-BAPI-SIGN':sig,'X-BAPI-TIMESTAMP':ts,'X-BAPI-RECV-WINDOW':rw,'Content-Type':'application/json'},body:bs,signal:AbortSignal.timeout(timeoutMs)});
  ...
  log('INFO','ORDER','ACK '+side+' '+symbol+'/'+category,{orderId:json.result?.orderId,ackMs:ackAt-sentAt});
  return {orderId:json.result?.orderId,sentAt,ackAt};
}
...
async function withRetry(fn,sym,max=2){const delays=[0,500];let lastErr;for(let i=0;i<max;i++){if(i>0)await new Promise(r=>setTimeout(r,delays[i]));try{const r=await fn();recordCircuitSuccess(sym);return r;}catch(e){lastErr=e;}}recordCircuitFailure(sym,lastErr?.message);throw lastErr;}
async function executeBothLegs(signal) {
  const t0=Date.now(); const asset=String(signal.asset||(signal.pair?.split('-')[0])||'BTC').toUpperCase(); const symbol=asset+'USDT';
  if (!ALLOWED_SYMBOLS.has(symbol)) throw new ExecError(ERR.ASSET_NOT_ALLOWED,symbol);
  if (isCircuitOpen(symbol)) throw new ExecError(ERR.CIRCUIT_OPEN,'Circuit OPEN for '+symbol);
  const rawQty=Number(signal.qty); const buyIsPerp=/perp|swap|linear/i.test(signal.buy_exchange||''); const sellIsPerp=/perp|swap|linear/i.test(signal.sell_exchange||'');
  const latency={t0,validationStart:t0};
  if (!buyIsPerp&&!sellIsPerp) {
    const isBuyBybit=(signal.buy_exchange||'').toLowerCase().includes('bybit'); const isSellBybit=(signal.sell_exchange||'').toLowerCase().includes('bybit');
    if (!isBuyBybit&&!isSellBybit) throw new ExecError('NO_BYBIT_LEG','no bybit leg');
    const side=isBuyBybit?'Buy':'Sell'; const refPrice=Number(isBuyBybit?signal.buy_price:signal.sell_price)||0;
    const norm=await normalizeAndValidateOrder('spot',symbol,rawQty,refPrice); latency.validationEnd=Date.now(); latency.orderSentAt=Date.now();
  ...
  if (!buyIsPerp&&sellIsPerp){spotSide='Buy';perpSide='Sell';}else if(buyIsPerp&&!sellIsPerp){spotSide='Sell';perpSide='Buy';}else{spotSide='Buy';perpSide='Sell';}
  ...
  if (!spotOk&&!perpOk) throw new ExecError(ERR.BOTH_LEGS_FAILED,'spot='+spotRes.reason?.message+' perp='+perpRes.reason?.message);
  if (!spotOk||!perpOk) log('ERROR','EXEC','PARTIAL FILL '+symbol+' MANUAL REVIEW',{spotOk,perpOk});
  return {spotOk,perpOk,spotOrderId:spotRes.value?.orderId,perpOrderId:perpRes.value?.orderId,symbol,spotSide,perpSide,mode:spotOk&&perpOk?'live':'live_partial',latency};
}
...
function authCheck(req,res){const t=((req.headers['authorization']||'').replace('Bearer ','').trim())||req.headers['x-droplet-secret'];if(t!==SECRET){res.writeHead(401,{'Content-Type':'application/json'});res.end(JSON.stringify({error:'unauthorized'}));return false;}return true;}
const server=http.createServer(async(req,res)=>{
  ...
  if (method==='GET'&&(url==='/balance'||url==='/api/balance')){if(!authCheck(req,res))return;try{const b=await fetchBalance();res.writeHead(200,{'Content-Type':'application/json'});res.end(JSON.stringify({...b,testnet:IS_TESTNET}));}catch(e){res.writeHead(500);res.end(JSON.stringify({error:e.message}));}return;}
  if (method==='GET'&&url.startsWith('/price')){if(!authCheck(req,res))return;const p=new URL(url,'http://x').searchParams;const sym=(p.get('symbol')||'').toUpperCase();const cat=(p.get('category')||'spot').toLowerCase();if(!sym){res.writeHead(400);res.end('{"error":"missing_symbol"}');return;}try{const r=await fetch(BYBIT_BASE+'/v5/market/tickers?category='+cat+'&symbol='+sym,{signal:AbortSignal.timeout(5000)});const j=await r.json();const price=parseFloat((j.result?.list?.[0]?.lastPrice)||0);res.writeHead(200,{'Content-Type':'application/json'});res.end(JSON.stringify({ok:true,symbol:sym,category:cat,price}));}catch(e){res.writeHead(500);res.end(JSON.stringify({error:e.message}));}return;}
...
Deno.serve(async (req) => {
  return new Response(ORDER_SERVER_SOURCE, {
    status: 200,
    headers: {
