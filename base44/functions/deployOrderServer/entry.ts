...
async function getOrderServerCode() {
  // Read order-server.mjs from the droplet-bot directory
  const response = await fetch('https://raw.githubusercontent.com/your-repo/main/droplet-bot/order-server.mjs');
  if (response.ok) {
    return await response.text();
  }
  // Fallback: embed the code directly (update this with actual order-server.mjs content)
  return `// order-server.mjs — Droplet execution server for Basis-Carry v3
  ...
  if (req.method === 'POST' && req.url === '/execute') {
    ...
    req.on('end', async () => {
      ...
      try {
        ...
        await reportResult({ signal_id: signal.signal_id, ok: false, error: safeErr });
        return;
      }

      res.writeHead(200, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ ok: true, ...result }));
      await reportResult({ signal_id: signal.signal_id, ok: true, ...result });
    });
    return;
  }
